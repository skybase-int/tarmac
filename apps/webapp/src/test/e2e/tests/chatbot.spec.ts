 ...
 * with chatbot terms acceptance. All API calls are mocked to avoid hitting
 * real backend (which has no test environment).
 *
 * Test scenarios covered:
 * - Happy path: wallet connected + terms accepted → association API called
 * - Returning user: terms already accepted → association triggered on load
 * - No wallet: terms accepted without wallet → no association call
 * - API responses: success, alreadyRecorded, 403, 500 errors
 * - Graceful degradation: API errors don't break chat functionality
 */
test.describe('Wallet Terms Association', () => {
  test('associates wallet when terms are accepted via modal', async ({ isolatedPage }) => {
    // Track wallet association requests
    ...
    // Initially return terms NOT accepted
    await isolatedPage.route('**/chatbot/terms/check', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    // Mock terms signing
    await isolatedPage.route('**/chatbot/terms/sign', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    // Mock wallet association endpoint - capture requests
    await isolatedPage.route('**/chatbot/terms/wallet', async route => {
      const body = route.request().postDataJSON();
    ...
    // Send a message to trigger 401 → terms modal flow
    const chatInput = isolatedPage.locator('textarea');
    await chatInput.fill('Hello');
    await chatInput.press('Enter');

    // Wait for the "Accept Terms" button to appear (from 401 response handling)
    // Note: The existing mock returns success, so we need to trigger the terms modal differently
    // Let's override the chat mock to return 401 for this specific flow
    await isolatedPage.route('**/chat', async route => {
      if (route.request().method() === 'POST') {
        await route.fulfill({
          status: 401,
    ...
    // Send another message to trigger 401
    await chatInput.fill('Trigger terms');
    await chatInput.press('Enter');

    // Wait for Accept Terms button
    const acceptTermsButton = isolatedPage.getByRole('button', { name: /accept terms/i });
    await expect(acceptTermsButton).toBeVisible({ timeout: 10000 });
    await acceptTermsButton.click();

    // Wait for terms modal
    const dialog = isolatedPage.getByRole('dialog');
    await expect(dialog).toBeVisible();

    // Now update the terms check mock to return accepted
    await isolatedPage.route('**/chatbot/terms/check', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ accepted: true, acceptanceId: 'test-id' })
      });
    });

    // Scroll to bottom and accept terms
    await dialog.getByRole('button', { name: /Scroll down/i }).click();
    await isolatedPage.waitForTimeout(500);

    // Find and click the accept button
    const acceptButton = dialog.getByRole('button', { name: /I accept/i });
    await expect(acceptButton).toBeVisible();
    await acceptButton.click();

    // Wait for modal to close and association to be triggered
  ...
  test('associates wallet on page load when terms are already accepted', async ({ isolatedPage }) => {
    // Track wallet association requests
    const walletAssociationRequests: { wallet: string }[] = [];

    // Setup mocks - terms already accepted
    await isolatedPage.route('**/chat', async route => {
      if (route.request().method() === 'POST') {
        await route.fulfill({
          status: 200,
    ...
    // Mock wallet association endpoint
    await isolatedPage.route('**/chatbot/terms/wallet', async route => {
      const body = route.request().postDataJSON();
      walletAssociationRequests.push(body);
      await route.fulfill({
    ...
    // Wait for the hook to trigger association
    await isolatedPage.waitForTimeout(2000);

    // Verify wallet association was called automatically
    expect(walletAssociationRequests.length).toBeGreaterThan(0);
    expect(walletAssociationRequests[0].wallet).toMatch(/^0x[a-fA-F0-9]{40}$/);
  });

  test('does not call association API when wallet is not connected', async ({ isolatedPage }) => {
    // Track wallet association requests
    const walletAssociationRequests: { wallet: string }[] = [];
  ...
  test('handles alreadyRecorded response gracefully', async ({ isolatedPage }) => {
    // Track requests
    const walletAssociationRequests: { wallet: string }[] = [];
    ...
    // Verify response appears (chat not broken)
    await expect(isolatedPage.getByText('Hello!')).toBeVisible({ timeout: 5000 });
  });

  test('handles 403 error gracefully - chat continues to work', async ({ isolatedPage }) => {
    // Track requests
    const walletAssociationRequests: { wallet: string }[] = [];
    ...
    await isolatedPage.route('**/chat', async route => {
      if (route.request().method() === 'POST') {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ response: 'Chat still works!', actions: [] })
        });
      } else {
        await route.continue();
      }
    ...
    // Return 403 Forbidden (backend not ready scenario)
    await isolatedPage.route('**/chatbot/terms/wallet', async route => {
      const body = route.request().postDataJSON();
      walletAssociationRequests.push(body);
      await route.fulfill({
    ...
    // Verify chat still works despite the error
    await isolatedPage.waitForSelector('textarea[placeholder]');
    const chatInput = isolatedPage.locator('textarea');
    await chatInput.fill('Test after error');
    await chatInput.press('Enter');

    // Chat should still function
    await expect(isolatedPage.getByText('Chat still works!')).toBeVisible({ timeout: 5000 });
  });

  test('handles 500 server error gracefully - chat continues to work', async ({ isolatedPage }) => {
    // Track requests
    const walletAssociationRequests: { wallet: string }[] = [];
    ...
    await isolatedPage.route('**/chat', async route => {
      if (route.request().method() === 'POST') {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ response: 'Chat works after 500!', actions: [] })
        });
      } else {
        await route.continue();
      }
    ...
    // Verify chat still works despite the server error
    await isolatedPage.waitForSelector('textarea[placeholder]');
    const chatInput = isolatedPage.locator('textarea');
    await chatInput.fill('Test after 500');
    await chatInput.press('Enter');

    // Chat should still function
    await expect(isolatedPage.getByText('Chat works after 500!')).toBeVisible({ timeout: 5000 });
  });

  test('handles network timeout gracefully', async ({ isolatedPage }) => {
    // Track requests
    const walletAssociationRequests: { wallet: string }[] = [];
    ...
    await isolatedPage.route('**/chat', async route => {
      if (route.request().method() === 'POST') {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ response: 'Chat works after timeout!', actions: [] })
        });
      } else {
        await route.continue();
      }
    ...
    await isolatedPage.route('**/chatbot/terms/wallet', async route => {
      walletAssociationRequests.push({ wallet: 'attempted' });
    ...
    // Verify chat still works despite network failure
    await isolatedPage.waitForSelector('textarea[placeholder]');
    const chatInput = isolatedPage.locator('textarea');
    await chatInput.fill('Test after network failure');
    await chatInput.press('Enter');

    // Chat should still function
    await expect(isolatedPage.getByText('Chat works after timeout!')).toBeVisible({ timeout: 5000 });
  });

  test('sends correct wallet address format in request', async ({ isolatedPage }) => {
    // Track the exact request payload
    let capturedPayload: { wallet: string } | null = null;
    ...
    expect(capturedPayload!.wallet).toBeDefined();

    // Verify wallet address is a valid Ethereum address format
    expect(capturedPayload!.wallet).toMatch(/^0x[a-fA-F0-9]{40}$/);

    // Verify it's the correct field name (not walletAddress)
    expect(capturedPayload).toHaveProperty('wallet');
    expect(capturedPayload).not.toHaveProperty('walletAddress');
  });

  test('does not make duplicate association calls on same page', async ({ isolatedPage }) => {
    // Track all association requests
    const walletAssociationRequests: { wallet: string }[] = [];
    ...
    // (the cache should prevent duplicate calls)
