...
import { isArbitrumChainId, isBaseChainId, isOptimismChainId, isUnichainChainId } from '@jetstreamgg/utils';
...
export function ConnectCard({ intent }: { intent: Intent }) {
  ...
  const isOptimismChain = isOptimismChainId(chainId);
  const isUnichainChain = isUnichainChainId(chainId);
    ...
    [Intent.SAVINGS_INTENT]: isBaseChain
      ...
      : isArbitrumChain
        ...
        : isOptimismChain
          ...
          : isUnichainChain
            ? t`Supplying stablecoins to the Savings feature on either Unichain or Ethereum is very similar. However, given that no native Sky Savings Rate module is currently deployed to Unichain, when transacting on Unichain you always interact with the Peg Stability Module (PSM) for conversions to/from USDS, sUSDS and USDC. The PSM on Unichain handles conversions programmatically, driven at your direction, between these pairs directly.`
