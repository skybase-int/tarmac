...
import { ActivationPositionRewardsCard } from './ActivationPositionRewardsCard';
...
export function ActivationPositionOverview({
  positionIndex
}: {
      ...
      <DetailSectionRow>
        <VStack className="gap-8">
          <HStack gap={2} className="scrollbar-thin w-full overflow-auto">
            ...
            />
            {data?.selectedReward && (
              <ActivationPositionRewardsCard rewardContractAddress={data.selectedReward as `0x${string}`} />
            )}
          </HStack>
          {(data?.selectedDelegate || data?.selectedReward) && (
            <HStack gap={2} className="w-full">
              {data?.selectedReward && <ActivationRewardCard selectedReward={data.selectedReward} />}
              {data?.selectedDelegate && <ActivationDelegateCard selectedDelegate={data.selectedDelegate} />}
            </HStack>
          )}
          <HStack gap={2} className="scrollbar-thin w-full overflow-auto">
            <StatsCard
