...
const ActivationRewardsOverviewRow = ({ contractAddress }: { contractAddress: `0x${string}` }) => {
  const {
...
export function ActivationRewardsOverview() {
    ...
    >
      <VStack className="space-y-8">
        {data?.map(({ contractAddress }) => (
          <ActivationRewardsOverviewRow key={contractAddress} contractAddress={contractAddress} />
        ))}
      </VStack>
    </LoadingErrorWrapper>
  );
