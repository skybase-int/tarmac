...
export function HistoryTableUiComponents({
  history,
  error,
  itemsPerPage = 5,
  errorText,
...
export function HistoryTable(props: Partial<HistoryTableProps>) {
  const errorText = props.errorText || 'There was an error fetching this data';
  const noWalletText = props.noWalletText || 'Please connect your wallet to see your history';
  const noTransactionsText = props.noTransactionsText || 'No transactions found';
  const transactionHeader = props.transactionHeader || 'Transactions';
  const typeHeader = props.typeHeader || 'Type';
