...
export function getStepTitle(step: ActivationStep, tab: 'left' | 'right'): MessageDescriptor {
  switch (step) {
    case ActivationStep.OPEN_BORROW:
      return tab === 'left' ? msg`Seal and Borrow` : msg`Unseal and pay back`;
    case ActivationStep.REWARDS:
      return msg`Select reward`;
    case ActivationStep.DELEGATE:
      return msg`Select a delegate`;
    case ActivationStep.SUMMARY:
...
export const activationApproveTitle: TxCardCopyText = {
...
export const activationApproveSubtitle: TxCardCopyText = {
...
export const activationApproveDescription: Record<string, MessageDescriptor> = {
  [TOKENS.mkr.symbol]: msg`Sealing MKR in the Activation Rewards module`,
  [TOKENS.sky.symbol]: msg`Sealing SKY in the Activation Rewards module`
};

export const repayApproveDescription: MessageDescriptor = msg`Repaying USDS in the Seal Rewards module`;

export function getActivationTitle(
  txStatus: Omit<TxStatus, TxStatus.CANCELLED>,
  flow: ActivationFlow
): MessageDescriptor {
  switch (txStatus) {
    case TxStatus.INITIALIZED:
      return flow === ActivationFlow.OPEN
...
export function getActivationSubtitle({
  flow,
  txStatus,
  collateralToLock,
  borrowAmount,
...
export function activationLoadingButtonText({
  txStatus,
  flow
}: {
  flow: ActivationFlow;
